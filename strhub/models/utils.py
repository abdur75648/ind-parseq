...
def create_model(experiment: str, pretrained: bool = False,learning_rate:float=0.0, **kwargs):
    print("Loading model from ", os.path.join(experiment,"checkpoints","last.ckpt"))
    try:
        with open(os.path.join(experiment,"config","config.yaml"), 'r') as f:
            config = yaml.load(f, yaml.Loader)['model']
        if learning_rate:
            print("Updating learning rate from ", config['lr']," to ", learning_rate)
            config['lr'] = learning_rate
        else:
            print("Keeping learning rate = ", config['lr'])
    ...
    if pretrained:
        try:
            ...
            checkpoint = torch.load(os.path.join(experiment,"checkpoints","last.ckpt"))
            model.load_state_dict(checkpoint['state_dict'])

        except KeyError:
            #raise InvalidModelError("No pretrained weights found for '{}'".format(experiment)) from None
            raise InvalidModelError("No pretrained weights found for '{}'".format(os.path.join(experiment,"checkpoints","last.ckpt"))) from None
    return model
