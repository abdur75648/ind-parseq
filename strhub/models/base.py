...
class BaseSystem(pl.LightningModule, ABC):
    ...
    def __init__(self, tokenizer: BaseTokenizer, charset_file: str,
...
class CrossEntropySystem(BaseSystem):
    ...
    def __init__(self, charset_file: str, #charset_train: str, charset_test: str,
                 batch_size: int, lr: float, warmup_pct: float, weight_decay: float) -> None:
        with open(charset_file,"r",encoding="utf-8") as file:
            lines = file.readlines()
        charset_train=""
        ...
        super().__init__(tokenizer, charset_file, batch_size, lr, warmup_pct, weight_decay)
